...
public class AOT {
  public static void main(String[] args) throws IOException {
    ...
    while (Q-- > 0) {
      int op = sc.nextInt();
      if (op == 1) {
        segTree.update_point(sc.nextInt(), sc.nextInt());
      } else {
        pw.println(segTree.query(sc.nextInt(), sc.nextInt()));
      }
    }
  ...
  static class SegmentTree {
    ...
    // void update_range(int left, int right, int val) // adds the value to the
    // given interval in O(log n)
    // {
    // update_range(1, 1, N, left, right, val);
    // }

    // void update_range(int node, int nodeStart, int nodeEnd, int left, int right,
    // int val) {
    // if (left > nodeEnd || right < nodeStart) // if the current node's interval
    // doesn't intersect with the required
    // // interval(they are disjoint), return
    // return;

    // if (nodeStart >= left && nodeEnd <= right) { // if the current node's
    // interval is completely covered by the
