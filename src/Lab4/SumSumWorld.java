...
public class SumSumWorld {
  ...
  static class FFT {
    ...
    public static Complex[] fft(Complex[] A, boolean invert) { // Length of A must be a power of 2
      ...
      // which are equal to w4 ^ 0, w4 ^ 1, w4 ^ 2, w4 ^ 3 (the (n/2)th roots of
      // unity)
      ...
      // which are equal to A0((w8 ^ 0) ^ 2),A0((w8 ^ 1) ^ 2), A0((w8 ^ 2) ^ 2),
      // A0((w8 ^ 3) ^ 2)
      // and are also equal to A0((w8 ^ 4) ^ 2),A0((w8 ^ 5) ^ 2), A0((w8 ^ 6) ^ 2),
      // A0((w8 ^ 7) ^ 2)
      // A1FFT is similar
    ...
    public static long[] multiply(int[] A, int[] B) {
      ...
      long[] intres = new long[N];
      for (int i = 0; i < N; i++) {
        intres[i] = (long) Math.round(res[i].real);
      }
